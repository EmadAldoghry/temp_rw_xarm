...
bool XArmPlanner::planCartesianPath(const std::vector<geometry_msgs::msg::Pose>& pose_target_vector)
{   
    double fraction = move_group_->computeCartesianPath(pose_target_vector, eef_step, 0.0, trajectory_); // jump_threshold = 0.0
    
    // <<< FIX: Check fraction to see if planning was successful
    bool success = (fraction > 0.9); // Consider plans that cover at least 90% of the path as successful

    if (!success) {
        RCLCPP_ERROR(node_->get_logger(), "planCartesianPath: plan failed, fraction = %lf", fraction);
    }
    is_trajectory_ = success; // Only mark as a valid trajectory if planning was successful
    if (success) {
        xarm_plan_.trajectory_ = trajectory_;
    }
...
{
    moveit::core::MoveItErrorCode code;
    // <<< FIX: Correctly check which plan to execute
    if (is_trajectory_) {
        // This is a cartesian path
        code = wait ? move_group_->execute(trajectory_) : move_group_->asyncExecute(trajectory_);
    } else {
